...
const Cart = () => {
  ...
  return (
    ...
    >
      <div
        onClick={() => dispatch(isVisibleAction(false))}
        id="slideover-bg"
      ...
      <div
        id="slideover"
        className={`flex flex-col w-full md:w-[35rem] bg-white h-full absolute right-0 duration-300 ease-out transition-all overflow-y-auto ${
          isVisible ? "" : "translate-x-full"
        } ${cartItems.length === 0 && "justify-between items-center"}`}
      >
        <div className="flex w-full justify-between pt-4 pb-3 px-4 border-b-2">
          ...
          <div
            onClick={() => dispatch(isVisibleAction(false))}
            className="cursor-pointer text-gray-600 flex items-center justify-center w-8 h-8"
          >
            <svg
              className="w-6 h-6"
              fill="none"
                ...
                <React.Fragment key={item.key}>
                  <div className="flex items-center justify-between border-b-2 shadow-sm">
                    ...
                    <div
                      ...
                      onClick={() => removeItem(item.key, item.quantity)}
                    >
                      <FontAwesomeIcon
                        icon={faCircleXmark}
                        size="xl"
