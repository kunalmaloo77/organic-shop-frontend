...
const Login = () => {
  ...
  const addUser = async (user) => {
    try {
      const res = await axios.post('https://organic-shop-backend.vercel.app/api/users', user, { withCredentials: true });
  ...
  const loginUser = async (user) => {
    try {
      const res = await axios.post('https://organic-shop-backend.vercel.app/api/auth/login/password', user, { withCredentials: true });
      console.log(res);
      if (res.status === 200) {
        setLoggedIn(true);
      }
  ...
  const checkAuthenticated = async () => {
    try {
      const res = await axios.get('https://organic-shop-backend.vercel.app/api/auth/login', { withCredentials: true });
      console.log('res->', res);
  ...
  const handlelogout = async () => {
    try {
      const res = await axios.delete('https://organic-shop-backend.vercel.app/api/auth/logout', { withCredentials: true });
      console.log('res->', res);
      if (res.status === 200) {
        setLoggedIn(false);
      }
