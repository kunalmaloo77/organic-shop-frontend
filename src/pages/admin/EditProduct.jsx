...
const EditProduct = () => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      key: productData?._id ?? "",
      title: productData?.title ?? "Grocery",
      name: productData?.name ?? "",
      price: productData?.price ?? "",
      sale: typeof productData?.sale === "boolean" ? productData.sale : false,
    ...
    onSubmit: async (values) => {
      try {
        ...
        // If a new file was selected, upload it. Otherwise keep existing image_url
        if (file && values.fileType && values.fileType.startsWith("image/")) {
          const {
            data: {
        ...
        // Use update endpoint for edits; fall back to create endpoint if not present
        const endpoint = "/admin/product/" + (values.key ? values.key : "");
        const response = await instance.patch(endpoint, values);
  ...
  const clearImage = () => {
    ...
    // clear formik fields related to image
    try {
